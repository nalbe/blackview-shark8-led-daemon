...
5. Notification "stuck armed" (LED never returns to charge): the logdr socket
   stayed ALIVE but went SILENT -- no "logdr closed" (recv never returned <=0,
   so the n<=0 reconnect path could not fire) and the reconnect gate required
   being idle, so an armed notification could never hand the LED back. Root
   cause was NOT a code bug: the events buffer gets flooded by log-system
   spam (auditd + am_compact/am_freeze etc.), logd drops/neglects the old
   reader and stops delivering notification_enqueue/cancel events to it.
   Hypothesis: installing the separate patch
   shark8-gsi-logspam-cosmetics (which silences the events-log spam) empties
   the flood and should prevent the silent logdr stall, so the daemon no
   longer gets stuck armed. Verified empirically: once the spam was killed,
   a fresh daemon disarmed correctly (cancel -> disarm -> charge). An
   experimental time-based logdr health-check reconnect was tried and then
   reverted (it treats a symptom, not the cause); testing the reverted build
   against the fixed-logspam system to confirm the stall no longer recurs.
