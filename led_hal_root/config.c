 ...
 *   [notify]    SHARED behavior for every app: notify_light_type
 *               (0=off 1=breathing 2=flashing 3=static),
 *               rise/hold/fall/offt (breath ms), notif_max_sec (0 = unlimited);
 *               default_color r,g,b applies only to apps without a
 *               [rules] entry (per-app color = [rules]; per-app type/breath
 *               is not supported)
 *
...
static void load_file(void)
{
    ...
    fclose(f);
    /* [led] logging -> logger toggle. Also keeps the file write path live
     * so a "logging on" is still visible on the very next log line. The
     * LOGI below intentionally comes after the toggle applies. */
