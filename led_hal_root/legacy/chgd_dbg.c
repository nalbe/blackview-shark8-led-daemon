...
#define RING_CHK_SEC     8         /* telephony state re-poll period */
 ...
 *   screen on    -> disarm (user sees the call anyway)
 * First state re-poll happens RING_CHK_SEC after entry: an instant
 * hang-up still gets a short glow instead of a zero-length flash. */
static time_t g_last_tele_chk;

static void arm_ring(void)
{
...
/* 1 = RINGING (any line), 0 = idle/offhook/unknown */
...
{
    if (tele_ringing()) {           /* state flipped after the ping */
...
static void handle_enqueue(struct notif_state *st, const char *pkg)
{
    ...
    if (is_dialer(pkg)) {
        ...
        /* not ringing right now: maybe the missed row lands slightly
...
int main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        if (g_fake_ring) {
            g_fake_ring = 0;
            if (!g_st.cur_pkg[0]) arm_ring();   /* rainbow test w/o call */
        }
        if (g_clear) {
            g_clear = 0;
        ...
        if (FD_ISSET(tfd, &rfds)) {
            ...
            if (g_st.cur_pkg[0] && !strcmp(g_st.cur_pkg, INCOMING_PKG)) {
                ...
                if (dt >= RING_CHK_SEC) {
                    ...
                    if (!tele_ringing()) {
                        LOGI("ring ended -> resolving outcome");
                        g_st.cur_pkg[0] = '\0';
                        apply_charge_leds();
