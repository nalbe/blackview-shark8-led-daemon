...
int g_tfd = -1;
int g_ld  = -1;
/* experimental health-check (disabled for test):
 * static time_t g_last_ld_rx;
 * #define LOGDR_STALL_SEC 20 */
...
static unsigned g_tag_cancel     = 27502;  /* AOSP notification_cancel  */
static unsigned g_tag_cancel_all = 27503;  /* AOSP notification_cancel_all */
static unsigned g_tag_canceled   = 27504;  /* AOSP notification_canceled   */
...
void ev_load_tags(void)
{
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (sscanf(line, "%d %127s", &id, name) == 2) {
            if (!strcmp(name, "notification_enqueue"))
                g_tag_enq = (unsigned)id;
            else if (!strcmp(name, "notification_cancel"))
                g_tag_cancel = (unsigned)id;
            else if (!strcmp(name, "notification_cancel_all"))
                g_tag_cancel_all = (unsigned)id;
            else if (!strcmp(name, "notification_canceled"))
                g_tag_canceled = (unsigned)id;
        }
    }
    fclose(f);
    LOGI("tags: enqueue=%u cancel=%u cancel_all=%u canceled=%u",
         g_tag_enq, g_tag_cancel, g_tag_cancel_all, g_tag_canceled);
}
...
/* Extract package name from notification_canceled key format: "user|pkg|id|tag|uid"
 * The key is a single string record with pipe-delimited fields. */
static int extract_pkg_from_key(const unsigned char *p, size_t n,
                                char *out, size_t on)
{
    for (size_t i = 0; i + 6 <= n; i++) {
        if (p[i] != 0x02) continue;
        unsigned len = le32(p + i + 1);
        if (len < 5 || len > 128 || i + 5 + len + 1 > n) continue;
        const unsigned char *s = p + i + 5;
        if (s[len] != 0x00) continue;
        /* key format: "user|pkg|id|tag|uid" - find first and second pipe */
        const unsigned char *first_pipe = NULL;
        const unsigned char *second_pipe = NULL;
        for (unsigned j = 0; j < len; j++) {
            if (s[j] == '|') {
                if (!first_pipe) first_pipe = s + j;
                else if (!second_pipe) { second_pipe = s + j; break; }
            }
        }
        if (!first_pipe || !second_pipe) continue;
        size_t pkg_len = second_pipe - first_pipe - 1;
        if (pkg_len < 3 || pkg_len >= on) continue;
        const unsigned char *pkg = first_pipe + 1;
        /* validate: must look like a package name */
        if (pkg[0] < 'a' || pkg[0] > 'z') continue;
        int dots = 0, ok = 1;
        for (unsigned j = 0; j < pkg_len; j++) {
            unsigned char c = pkg[j];
            int alnum = (c >= 'a' && c <= 'z') || (c >= '0' && c <= '9') ||
                        (c >= 'A' && c <= 'Z') || c == '.' || c == '_';
            if (!alnum) { ok = 0; break; }
            if (c == '.') dots++;
             ...
             char *pkg, size_t pkgn)
{
    ...
    if (left < 6) return EV_NONE;
    unsigned tag = le32(p);
    int ev = EV_NONE;
    if (tag == g_tag_enq)              ev = EV_ENQUEUE;
    else if (tag == g_tag_cancel)      ev = EV_CANCEL;
    else if (tag == g_tag_cancel_all)  ev = EV_CANCEL;
    else if (tag == g_tag_canceled)    ev = EV_CANCEL;
    else return EV_NONE;

    /* notification_canceled embeds pkg in the key "user|pkg|id|tag|uid" */
    if (tag == g_tag_canceled) {
        if (!extract_pkg_from_key(p + 4, left - 4, pkg, pkgn)) return EV_NONE;
    } else {
        if (!extract_pkg(p + 4, left - 4, pkg, pkgn)) return EV_NONE;
    }
    return ev;
}
...
/* notification_cancel dispatch: disarm if the cancelled package matches
 * the currently armed notification. This is the primary event-driven
 * disarm trigger - no polling needed. */
static void cancel_dispatch(struct notif_state *st, const char *pkg)
{
    if (st->cur_pkg[0] && !strcmp(st->cur_pkg, pkg)) {
        LOGI("cancel_dispatch: %s matches armed pkg", pkg);
        disarm_notification(st, "notification_cancel");
    }
}
...
int main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        if (g_ld >= 0 && FD_ISSET(g_ld, &rfds)) {
            ...
            if (n <= 0) {
                ...
                if (g_verbose && (size_t)n >= 32) {
                    ...
                    LOGI("ldpkt n=%zd hdr=%u tag=%u ev=%d pkg=%s",
                ...
                else if (hit == EV_CANCEL)
                    cancel_dispatch(&g_st, pkg);
            }
        }
        ...
        if (FD_ISSET(tfd, &rfds)) {
             ...
             * experimental health-check (disabled for test):
             * if (g_ld >= 0) {
             *     long stall = (long)(time(NULL) - g_last_ld_rx);
             *     if (stall > LOGDR_STALL_SEC) {
             *         LOGI("logdr stalled %lds, reconnecting", stall);
             *         close(g_ld);
             *         g_ld = -1;
             *         g_last_ld_rx = time(NULL);
             *     }
