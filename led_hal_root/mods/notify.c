...
void arm_notification_ex(struct notif_state *st, const char *pkg, int test)
{
    ...
    rgb_for(pkg, &r, &g, &b);
    LOGI("arm detect: %s -> rgb=%d,%d,%d (rules|internal|default)", pkg, r, g, b);
...
static void notify_tick(void)
{
    ...
    /* screen-on disarm is deliberately removed - the notification owns the
     * LED while it is alive, on or off screen. The channel is handed back
     * to the idle owner (charge) via notification_cancel or the cap. */
}

/* Adaptive heartbeat: return until the next real piece of work so the
 * core can sleep:
 *   a parked (screen-on grace) pending notify -> 1s, it is transient
 *   finite notif_max_sec cap                 -> wake exactly when it expires
 *   permanent led (cap=0)                    -> quiet, disarm is event-driven
 *                                               (cancel / uevent), safety
 *                                               only via watchdog */
static long notify_next_wake(void)
{
    ...
    return 0;                  /* permanent: no timer needed, event-driven disarm */
}
