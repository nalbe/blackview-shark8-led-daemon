...
files). Adding a feature never touches the core:
  ...
  mods/ (extension files, pick and choose):
    ...
    voip.c      messenger (VoIP) call rainbow (a timer MODE): a
                messenger that is actually in a call lights the
                rainbow via the audio-policy voice channel
...
Notifications (screen OFF only), colors from led.conf [rules]:
  ...
  org.telegram.messenger                    magenta       255,0,255
...
Messenger / VoIP call (Telegram/WhatsApp/Viber/Signal/Snapchat/Duo):
  when a messenger is actually in a live call (audio-policy voice
  channel IN_COMMUNICATION, or an unanswered IN_RINGTONE / a self-
  managed telecom call), its LED lights the same rainbow instead of
  the ordinary [rules] color. Chat messages keep their [rules] color.
  Config [voip]: max_sec (grace timeout, default 2), packages
  (comma list overrides the messenger set).
